...
class DeformablePoseTransformer(nn.Module):
    ...
    def __init__(self, cfg, backbone, transformer, **kwargs):
        ...
        self.num_queries = extra.NUM_QUERIES  ## 100 query的数目 与DETR中设置的一样
        self.num_classes = cfg.MODEL.NUM_JOINTS ## 关键点的数目，即为类别
        self.num_feature_levels = extra.NUM_FEATURE_LEVELS ## 4
        self.backbone = backbone
        self.transformer = transformer
        hidden_dim = transformer.d_model   ## 隐变量的维度
        self.class_embed = nn.Linear(hidden_dim, self.num_classes + 1) ## 类别编码 由隐变量维度到输出类别维度+1 表示增加了背景
        self.kpt_embed = MLP(hidden_dim, hidden_dim, 2, 3)  ## MLP 多层全连接 参数依次为输入层神经元的个数，隐藏层神经元的个数，输出层神经元的个数以及层数
    ...
    def forward(self, x):
        features, pos = self.backbone(x)  ## 特征图及位置编码
